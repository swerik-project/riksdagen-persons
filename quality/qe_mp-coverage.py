...
def main(args):
    ...
    baseline_df = pd.read_csv("test/data/baseline-n-mps-year.csv")
    baseline_df['year'] = baseline_df['year'].apply(lambda x: str(x)[:4])

    dates = pd.read_csv("test/data/session-dates.csv", sep=";")
    ...
    dates.to_csv("quality/estimates/mp-coverage/coverage.csv", index=False, sep=";")
    ...
    total = len(dates)
    with open("quality/estimates/mp-coverage/coverage-summary.txt", "w+") as out:
